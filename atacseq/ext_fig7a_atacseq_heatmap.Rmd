...
decoder.data = subset(decoder.data, !(Condition %in% c("P10d", "P1d", "P5d", "M")))
decoder.data = subset(decoder.data, !(type %in% c("effector")))
decoder.data$time <- factor(decoder.data$time, levels = c("0hr", "6hr", "12hr", "24hr", "5d", "7d", "10d", "14d", "21d", "28d", "35d", "60d"))
...
comps = c("N_vs_L6hr", "L6hr_vs_L12hr", "L12hr_vs_L1d", "L1d_vs_L5d", "L5d_vs_L7d", "L7d_vs_L14d", "L14d_vs_L21d", "L21d_vs_L28d", "L28d_vs_L35d", "L35d_vs_L60d")
for (comp in comps){
...
write.table(data.frame(df[,c("chr", "start", "end")], ID=df$peak, k=df$k), "ext_fig7a_atacseq_heatmap.bed", sep="\t", quote=F, col.names=F, row.names = F)
```
...
```{r onCluster, eval=F}
# cd /home/paz2005/archive/2023_01_mary_philip/figures_for_paper_with2017_lesion
library(profileplyr)
library(data.table)
library(BiocParallel)
bigwigfiles =  c("N.bw", "L6hr.bw", "L12hr.bw", "L24hr.bw", "L5.bw", "L7.bw", "L10.bw","L14.bw", "L21.bw", "L28.bw", "L35.bw","L60.bw")
all_profiles =  BamBigwig_to_chipProfile(signalFiles = bigwigfiles,
                                         testRanges ="./ext_fig7a_atacseq_heatmap.bed",
                                         style = "point",
                                         format = "bigwig",
                                         distanceAround = 1000, 
...
saveRDS(merged, "ext_fig7a_atacseq_heatmap.RDS")
```



```{r hm_clsrow_dend_reorder_f_row_annot, fig.width=6.5, fig.height=7, fig.path='ext_fig7a_atacseq_heatmap/', dev=c('png')}
all = readRDS("ext_fig7a_atacseq_heatmap.RDS")
peaks = df
all = all[df$peak,]
...
names =   c("N.bw", "L6hr.bw", "L12hr.bw", "L24hr.bw", "L5.bw", "L7.bw", "L10.bw","L14.bw", "L21.bw", "L28.bw", "L35.bw","L60.bw")
...
ha = ComplexHeatmap::HeatmapAnnotation(
  foo = ComplexHeatmap::anno_block(
    ...
    labels_gp = gpar(col = rep("white", 12))
  )
)
