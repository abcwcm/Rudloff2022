---
title: Ext. Fig. 5b
subtitle: " PCA of chromatin accessibility of TCRTAG from 6h to M in infection model."
...
decoder.data.sub = subset(decoder.data, type %in% c("effector", "naive", "memory"))
...
ggplot(plotData, aes(PC1, PC2)) + geom_point(size = 5, aes(color = Condition), show.legend = T) + theme_bw(base_size =18)   + xlab( paste0("PC1: ", round(c1p, digits=2), "% variance")) + ylab( paste0("PC2: ", round(c2p, digits=2), "% variance")) + theme(legend.position="bottom")   + geom_point(shape = 1,size=5,colour = "black") +  scale_color_manual(values=c("naive" = rgb2hex(99,99,99),
                             "E6hr" = rgb2hex(204,236,230),
                             "E12hr" = rgb2hex(153,216,201),
                             "E24hr" = rgb2hex(102,194,164),
                             "E5" = rgb2hex(35,139,69),
                             "E7" = rgb2hex(35,139,69),
                             "M" = rgb2hex(0,68,27)), name="")   + theme(plot.background = element_blank(), panel.grid.minor = element_blank(), panel.grid.major = element_blank()) + theme(legend.position ="none") + geom_label_repel(data=unique(out.dt[,c("x_center", "y_center", "Condition")]), aes(x = x_center, y = y_center, label = Condition), inherit.aes = FALSE, fontface = "plain", size = 4, segment.color = 'transparent') 
...
ggplot(plotData, aes(PC1, PC2)) + geom_point(size = 5, aes(color = Condition), show.legend = T) + theme_bw(base_size =18)   + xlab( paste0("PC1: ", round(c1p, digits=2), "% variance")) + ylab( paste0("PC2: ", round(c2p, digits=2), "% variance")) + theme(legend.position="bottom")   + geom_point(shape = 1,size=5,colour = "black") +  scale_color_manual(values=c("naive" = rgb2hex(99,99,99),
                             ...
                             "E5" = rgb2hex(35,139,69),
                             "E7" = rgb2hex(35,139,69),
                             "M" = rgb2hex(0,68,27)), name="")   + theme(plot.background = element_blank(), panel.grid.minor = element_blank(), panel.grid.major = element_blank()) + theme(legend.position ="none") 
```
