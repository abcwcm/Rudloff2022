---
title: Ext. Fig. 5a
subtitle: "Chromatin accessibility heatmap showing 6h to memory (M; 35d) in our Listeria infection model."
...
lfcTHR = 1
effector_list <- list(E6hr_vs_N=subset(dbs.all.sig$E6hr_vs_N, abs(log2FoldChange) > lfcTHR),
                      E12hr_vs_E6hr=subset(dbs.all.sig$E12hr_vs_E6hr, abs(log2FoldChange) > lfcTHR),
                      E24hr_vs_E12hr=subset(dbs.all.sig$E24hr_vs_E12hr,  abs(log2FoldChange) > lfcTHR),
                      E5_vs_E24hr=subset(dbs.all.sig$E5_vs_E24hr,  abs(log2FoldChange) > lfcTHR),
                      E7_vs_E5=subset(dbs.all.sig$E7_vs_E5,  abs(log2FoldChange) > lfcTHR),
                      M_vs_E7=subset(dbs.all.sig$M_vs_E7,  abs(log2FoldChange) > lfcTHR))



all.sig.rows = lapply(names(effector_list), function(x){
  effector_list[[x]] %>% as.data.frame() %>% row.names()
}) %>% invisible()

all.sig.rows = unique(unlist(all.sig.rows))

naive.decoder = subset(decoder.data, type == "naive")
e6hr.decoder = subset(decoder.data, type == "effector" & time == "6hr")
e12hr.decoder = subset(decoder.data, type == "effector" & time == "12hr")
e24hr.decoder = subset(decoder.data, type == "effector" & time == "24hr")
e5.decoder = subset(decoder.data, type == "effector" & time == "5d")
e7.decoder = subset(decoder.data, type == "effector" & time == "7d")
m.decoder = subset(decoder.data, type == "memory")
...
```{r kmeans_then_save_hm_effector, eval=F,  warning=FALSE, cache=TRUE,comment=FALSE, echo=FALSE,  fig.width=16, fig.height=10}
k=6
...
write.xlsx(ordered_mat_gr_anot_df, file="hm_effector_annotatedPeaks.xlsx")
```

```{r heatmap_deseqNorm, eval=T,  warning=FALSE,  echo=FALSE,  fig.width=10, fig.height=8}
ordered_mat_in = read.xlsx("hm_effector_annotatedPeaks.xlsx")
row.names(ordered_mat_in) <- ordered_mat_in$peak
ordered_mat = ordered_mat_in[,7:28]
...
pheatmap::pheatmap(ordered_mat, scale="none", show_rownames=F, fontsize = 10, cellwidth =20,  main = "DAPs across all contrasts",  annotation_col = annot_col,viridis::viridis(50), silent = F , cluster_rows=F, cluster_cols = F, gaps_col = c(nrow(naive.decoder),
                                 nrow(naive.decoder)+nrow(e6hr.decoder),
                                 nrow(naive.decoder)+nrow(e6hr.decoder)+nrow(e12hr.decoder),
                                 nrow(naive.decoder)+nrow(e6hr.decoder)+nrow(e12hr.decoder)+nrow(e24hr.decoder),
                                 nrow(naive.decoder)+nrow(e6hr.decoder)+nrow(e12hr.decoder)+nrow(e24hr.decoder)+nrow(e5.decoder),
                                  nrow(naive.decoder)+nrow(e6hr.decoder)+nrow(e12hr.decoder)+nrow(e24hr.decoder)+nrow(e5.decoder)+nrow(e7.decoder)),
...
write.table(data.frame(ordered_bed[,1:3], ID=ordered_bed$peak, km=ordered_bed$km), "extfig5_atac_heatmap_effector.bed", sep="\t", quote=F, col.names=F, row.names = F)

all_profiles =  BamBigwig_to_chipProfile(c("N.bw", "E6hr.bw", "E12hr.bw", "E24hr.bw", "E5.bw", "E7.bw", "M.bw"),testRanges ="extfig5_atac_heatmap_effector.bed",style = "point",format = "bigwig",distanceAround = 1000)

all = cbind(assay(all_profiles, 1),assay(all_profiles, 2),assay(all_profiles, 3), assay(all_profiles, 4), assay(all_profiles, 5), assay(all_profiles, 6), assay(all_profiles, 7))
...
ha =  ComplexHeatmap::HeatmapAnnotation(foo =  ComplexHeatmap::anno_block(gp = gpar(fill = c(rgb2hex(99,99,99),  rgb2hex(204,236,230),rgb2hex(153,216,201),rgb2hex(102,194,164), rgb2hex(35,139,69),  rgb2hex(35,139,69),rgb2hex(0,68,27))), labels = unique(rep(gsub(".bw", "", metadata(all_profiles)$names), each=100)),labels_gp = gpar(col = c("white", "black", "black", "black", rep("white", 3)))))
